...
from config import list_documents # Demo functionality
...
app.add_middleware(
    ...
    allow_credentials=True, # Allows cookies to be sent in cross-origin requests, which is necessary for session management.
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def get_documents():
    docs = list_documents()
    return {"documents": docs} # Endpoint to list available documents in Azure Blob Storage.
# DEMO END.

@app.get("/api/history/{session_id}")
async def get_history(session_id: str):
    history = manager.get_history(session_id)
    return {"history": history} # Endpoint so the frontend can fetch the full conversation history for a given session_id.
